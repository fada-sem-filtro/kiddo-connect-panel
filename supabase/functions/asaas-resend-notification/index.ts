...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: inv } = await svc.from("invoices").select("asaas_payment_id").eq("id", invoice_id).eq("creche_id", creche_id).maybeSingle();
    if (!inv) return json({ error: "Cobrança não encontrada" }, 404);

    const res = await asaasFetch(cred.apiKey, cred.env, `/payments/${inv.asaas_payment_id}/notifications/resendByCustomer`, { method: "POST" });
    if (!res.ok) return json({ error: "Falha ao reenviar notificação", details: res.data }, 400);
    return json({ ok: true });
  } catch (e) {
    console.error("asaas-resend-notification", e);

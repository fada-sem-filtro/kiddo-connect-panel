...
export default function OrcamentosPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const { data: orc } = await supabase.from("orcamentos").select("*").order("created_at", { ascending: false });
...
Equipe Agenda Fleur
      ...
      await supabase.functions.invoke("send-orcamento-email", {
        body: {
          ...
          subject: `Apresentação do Sistema Escolar - Agenda Fleur`,
          text: emailBody,
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-sm text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <School className="w-3.5 h-3.5" /> {orc.escola}
                          </span>
                          <span className="flex items-center gap-1">
                            <MapPin className="w-3.5 h-3.5" /> {orc.cidade}
                          </span>
                          <span className="flex items-center gap-1">
                            <Mail className="w-3.5 h-3.5" /> {orc.email}
                          </span>
                          {orc.telefone && (
                            <span className="flex items-center gap-1">
                              <Phone className="w-3.5 h-3.5" /> {orc.telefone}
                            </span>
                          )}
                          {orc.num_alunos && (
                            <span className="flex items-center gap-1">
                              <Users className="w-3.5 h-3.5" /> {orc.num_alunos} alunos
                            </span>
                          )}
                        </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div className="bg-muted/50 rounded-xl p-3 text-sm">
              <p>
                <strong>Escola:</strong> {selectedOrcamento?.escola}
              </p>
              <p>
                <strong>Cidade:</strong> {selectedOrcamento?.cidade}
              </p>
              {selectedOrcamento?.num_alunos && (
                <p>
                  <strong>Alunos:</strong> {selectedOrcamento.num_alunos}
                </p>
              )}
            </div>
            <Textarea

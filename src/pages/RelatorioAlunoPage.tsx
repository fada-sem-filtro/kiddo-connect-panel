...
export default function RelatorioAlunoPage() {
  ...
  const crecheInfo = isAdmin ? selectedCreche : userCreche;

  useEffect(() => {
    if (!crecheId) { setTurmas([]); setCriancas([]); return; }
    const fetch = async () => {
      const { data: t } = await supabase.from('turmas').select('id, nome').eq('creche_id', crecheId);
      setTurmas(t || []);
  ...
  const exportarPDF = async () => {
    ...
    await exportAlunoRelatorioPDF({
      ...
      crecheNome: crecheInfo?.nome || 'Escola',
      logoUrl: (crecheInfo as any)?.logo_url,
      crecheEndereco: (crecheInfo as any)?.endereco,
      crecheTelefone: (crecheInfo as any)?.telefone,
      crecheEmail: (crecheInfo as any)?.email,
      ...
      <div className="space-y-6">
        ...
        {!crecheId && isAdmin ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>Selecione uma escola para gerar relatórios</p>
          </div>
        ) : (
          <>
            {/* Filters */}

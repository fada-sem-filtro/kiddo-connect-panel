...
import { Utensils, Moon, Gamepad2, BookOpen, Droplets, MoreHorizontal, Pill, DoorOpen } from "lucide-react";
...
export function EventCard({ evento }: EventCardProps) {
  ...
  const renderDetails = () => {
    ...
    return details.length > 0 ? <div className="flex flex-wrap gap-1 mt-2">{details}</div> : null;
  };

  return (
    <div className={cn("rounded-2xl p-4 animate-fade-in kawaii-hover", eventStyles[evento.tipo] || "event-card-other")}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {renderDetails()}
          {evento.observacao && <p className="text-sm text-foreground/80 mt-2 line-clamp-2">{evento.observacao}</p>}
        </div>
      </div>
    </div>
  );
